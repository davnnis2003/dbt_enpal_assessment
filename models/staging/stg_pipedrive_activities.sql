{{ config(
    materialized='view',
    schema='staging',
    alias='stg_pipedrive_activities'
) }}
WITH
    source AS (
        SELECT
    ...
    renamed AS (
        SELECT
            ...
            cast(due_to AS timestamp) AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Berlin' AS due_at
        FROM
            source
    ),
    deduplicated AS (
        SELECT
            *,
            row_number() OVER (PARTITION BY activity_id ORDER BY due_at DESC) AS row_num
        FROM
            renamed
    )
SELECT
